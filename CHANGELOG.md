...
- Adds routing: compute routes with the MapTiler Routing API and draw them on the map.
  ...
  - The transport switcher follows TabItemFill: the selected tab is a white card lifted out of the tinted rail, with the panel's own text colour at a heavier weight rather than a colour of its own, and the icons keep one grey in every state.
  ...
  - The turn-by-turn view follows RoutingPanel/Detail: the way back and the heading share a tinted bar, and each step is a 24px maneuver icon beside its instruction, with the distance under it, separated by hairlines.
  - Reordering a stop shows what it is doing: the row itself travels under the cursor, a placeholder holds its place, and a line marks the gap it would drop into.
